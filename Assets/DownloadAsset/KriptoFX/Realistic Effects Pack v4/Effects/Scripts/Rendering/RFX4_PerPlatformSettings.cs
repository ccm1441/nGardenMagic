...
public class RFX4_PerPlatformSettings : MonoBehaviour
{
    ...
    void Update()
    {
        var cam = GameManager.GetInstance().camera;
        Legacy_Rendering_Check(GameManager.GetInstance().camera);
    }
    ...
    void OnDisable()
    {
        if (RenderMobileDistortion && !DisableOnMobiles && isMobile)
        {
            ...
            var mobileDistortion = GameManager.GetInstance().camera.GetComponent<RFX4_MobileDistortion>();
